...
function MedicineForm({
  ...
  const field = (
    ...
    <div className="space-y-1.5">
      ...
      {children}
      {touched[key] && errors[key] && <p className="text-xs text-destructive">{errors[key]}</p>}
    </div>
  );
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {field(
          ...
          <Input
            type="number"
            step="0.1"
            value={form.minStorageHumidity ?? ''}
            onChange={(e) =>
              set('minStorageHumidity', e.target.value ? Number(e.target.value) : undefined)
            }
          />,
        )}
        {field(
          ...
          <Input
            type="number"
            step="0.1"
            value={form.maxStorageHumidity ?? ''}
            onChange={(e) =>
              set('maxStorageHumidity', e.target.value ? Number(e.target.value) : undefined)
            }
          />,
        )}
        <div className="space-y-1.5">
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<MedicineDto> }) => {
      ...
      return medicineApi.update(
        id,
        Object.entries(patchData).map(([path, value]) => ({
          op: 'replace',
          path: `/${path}`,
          value,
        })),
