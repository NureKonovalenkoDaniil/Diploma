...
- Причина: Потреба адаптувати мобільний додаток під нову архітектуру (multi-tenancy, DTO, Lifecycle), зробити його стабільнішим та зручнішим для цільової аудиторії (домашніх користувачів).
...
Мобільний додаток призначений для **домашніх користувачів (User role)** і повинен бути спрощений:

- Перегляд препаратів (фільтр за статусом, пошук)
- Перегляд сповіщень (із позначенням як прочитане)
- Умови зберігання з графіками (температура, вологість за 24 години)
- Профіль та налаштування (мова, темна тема)
- Логін/Логаут

**НЕ буде у мобільному:**

- Добавлення/редагування/видалення препаратів (лише у web)
- Управління пристроями (лише у web для админів)
- Журнал аудиту (лише у web)
...
**Функціональність для домашніх користувачів:**

- Dashboard з препаратами (фільтр, пошук)
- Деталі препарату (read-only)
- Сповіщення (позначення як прочитане)
- Графіки умов зберігання (через бібліотеку Vico)
- Профіль користувача
...
- Функціональний мобільний додаток для домашніх користувачів
- Модерна архітектура (MVVM, DI, Compose)
- Добра UX (Material 3, темна тема, responsive)
- Готовий до демонстрації на защиті диплома
...
### Запис 16 — Відкат стану до коміту 2c92abac8c0a39485ea7eb6352945dfcc366d7ae (виконано 2026-05-11)

- Дата: 2026-05-11
- Завдання: Виконати відкат поточного стану репозиторію до зафіксованого стану коміту `2c92abac8c0a39485ea7eb6352945dfcc366d7ae`.
- Переглянуті файли / модулі:
  - Backend: `WebApp/MedicationManagement` (контролери та пов’язані зміни у робочому дереві)
  - Frontend: `Frontend/src/pages`, `Frontend/src/components/layout`, `Frontend/src/contexts`
  - Mobile: `Mobile/MedicationManagement/app/src/main/java/com/example/medicationmanagement`
  - Проєктна пам’ять: `PROJECT_CONTEXT.md`
- Основні висновки:
  1. Робоче дерево успішно приведено до снапшоту коміту `2c92abac8c0a39485ea7eb6352945dfcc366d7ae`.
  2. Відкат виконано як окремий поточний коміт у гілці, без переписування історії.
  3. Перед відкатом підтверджено проходження backend unit/integration тестів; frontend build виявив наявну проблему типізації тестових matcher-ів (`toBeInTheDocument`), що не пов’язана безпосередньо з відкатом.
- Що потрібно робити далі: За потреби виконати окрему стабілізацію frontend тестового оточення (Vitest + jest-dom типи) після підтвердження цільового стану відкату.
- Статус: ✅ Відкат до зазначеного коміту виконано.
