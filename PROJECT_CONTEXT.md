...
- ✅ Розмежування ролей (Administrator, Manager, User, Device). **Увага:** згідно з останнім оновленням, `User` має всі права `Administrator` та `Manager` у межах своєї організації, окрім створення нових менеджерів. Це зроблено для того, щоб зробити мобільний та веб-додатки повноцінними для домашніх користувачів (які отримують роль `User` при реєстрації).
