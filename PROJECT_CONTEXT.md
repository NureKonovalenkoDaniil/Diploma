...
### [ВИКОНАНО 2026-07-04] Оновлення та уніфікація форм створення/редагування пристроїв та локацій (мобільний застосунок)

- **Діагностика SMTP SendGrid**: Проведено детальний аналіз SMTP-підключення до `smtp.sendgrid.net:587`. TCP-з'єднання з сервером встановлюється успішно (сервер відповідає `220 SG ESMTP`), але обривається при спробі автентифікації (`The connection was closed`). Встановлено, що проблема полягає в блокуванні облікового запису користувача або його API-ключа на боці SendGrid (Compliance Review / Suspension).
- **Створення та редагування локацій зберігання (Storage Locations)**:
  - Повністю відмовилися від модального діалогу `MaterialAlertDialogBuilder` на користь окремої Activity (`EditStorageLocationActivity.kt` та макет `activity_edit_storage_location.xml`) для уніфікації інтерфейсу зі сторінками створення та редагування препаратів та пристроїв.
  - У `StorageLocationsFragment` перенаправлено логіку додавання та редагування на запуск нової `EditStorageLocationActivity` з передачею параметрів через `Intent`.
- **Впровадження ExposedDropdownMenu та темна тема**:
  - Замінено застарілі `Spinner` компоненти на сучасні `AutoCompleteTextView` з `TextInputLayout.OutlinedBox.ExposedDropdownMenu` на всіх екранах: додавання пристрою (`activity_add_device.xml`), редагування пристрою (`activity_edit_device.xml`) та додавання/редагування локації зберігання (`activity_edit_storage_location.xml`).
  - Видалено захардкоджені кольори фону в XML розмітках, переведено макети на використання системних темних/світлих кольорів (`?attr/colorSurface`), а також додано `MaterialToolbar` з підтримкою кнопки "Назад". Це забезпечило 100% сумісність із темною темою додатку.
  - Реалізовано динамічне наповнення списків локацій та пристроїв (через API) та коректний вибір початкових значень (включаючи обробку "Unassigned").
  - Додано нові локалізовані ресурси (UA/EN) для типів локацій та дій з пристроями у `strings.xml`.
...
### [ВИКОНАНО 2026-07-04] Оновлення фонових сервісів перевірки термінів придатності та низького запасу медикаментів

- **Генерація нових типів сповіщень (Backend):**
  - **Сповіщення про низький запас**: Додано фоновий аналіз кількості препаратів. У разі зниження залишків нижче порогового значення (`Business:LowStockThreshold`, за замовчуванням 10 шт.) система створює сповіщення `NotificationType.LowStock`.
  - **Сповіщення про прострочені препарати**: Реалізовано генерацію критичного сповіщення `NotificationType.Expiry` ("🚨 Термін придатності минув"), коли термін придатності ліків добігає кінця або вже минув.
- **Оптимізація інтервалу перевірок**: Інтервал запуску фонового сервісу `ExpiryNotificationService` змінено з 24 годин на **15 секунд**. Це дозволяє миттєво бачити сповіщення на фронтенді та мобільному клієнті відразу після запуску або додавання проблемного препарату.
- **Дедуплікація сповіщень**: Впроваджено перевірку у базі даних перед відправкою, яка обмежує створення однотипних сповіщень про термін придатності чи низький запас до одного разу на добу для кожного конкретного препарату.
- **Рефакторинг типів сповіщень на фронтенді**: Видалено невикористовуваний тип фільтрації «Система» (`System`), оскільки в бекенд-системі немає такого типу. Щоб не перевантажувати інтерфейс дублюючими категоріями (оскільки для відхилень умов є тип «Порушення»), фільтр за типом `IncidentCreated` також не додавався до випадаючого списку.
- **Уніфікація фільтрації сповіщень у мобільному додатку**: Перенесено логіку фільтрації з веб-фронтенду на мобільний клієнт (у `NotificationsFragment.kt`). Замінено нефункціональний фільтр «Інциденти» (`chipIncident`) на точні фільтри: «Порушення» (`StorageViolation`), «Норма» (`StorageRestored`), «Термін» (`Expiry`) та «Запас» (`LowStock`). Оновлено файли локалізації `strings.xml` (UA/EN) та XML-розмітку екрану сповіщень.
- **Виправлення та уніфікація фільтрації журналу аудиту (мобільний застосунок)**: Повністю перероблено екран аудиту логів (`AuditLogFragment.kt`, `AuditLogViewModel.kt` та макет `fragment_audit_log.xml`). Замість некоректної локальної фільтрації за `entityType` (яка не повертала результатів, бо більшість подій мають `null` у базі), реалізовано повноцінну серверну фільтрацію. Додано поля вводу користувача (`log_user`) та дії (`log_action`), інтегровано зручний `MaterialDatePicker` для вибору дат ("З" та "По") та кнопки «Застосувати»/«Скинути». Параметри передаються безпосередньо у запит до API, що забезпечує повну відповідність функціоналу веб-фронтенду.
- **Оновлення логіки та форми створення менеджера (мобільний застосунок)**: Додано нове поле введення «ID організації» (`Organization ID`) у діалогове вікно створення менеджера (`dialog_create_manager.xml`, `UsersFragment.kt` та `UsersViewModel.kt`). За замовчуванням це поле заповнюється `organizationId` поточного авторизованого адміністратора (отриманим через `RoleHelper.getOrganizationId`), але доступне для зміни вручну. Це повністю дублює поведінку веб-фронтенду і дозволяє створювати менеджерів для будь-якої організації. Помилка `409 Conflict: User already exists!` виникала через спробу створити вже зареєстрованого на бекенді користувача, тепер логіку валідації/інтерфейсу успішно приведено у відповідність до веб-версії.
- **Підвищення стійкості поштової підсистеми (Backend)**: Метод `SendAsync` у `SmtpEmailSender.cs` тепер обгорнутий у блок `try-catch`. Завдяки цьому будь-які виключення під час SMTP-надсилання (наприклад, коли у локальному середовищі не налаштований або недоступний поштовий сервер) логуються, але не переривають основну транзакцію. Це усунуло помилку `500 Internal Server Error` при повторному створенні щойно видаленого менеджера та під час реєстрації нових користувачів, коли користувач успішно створювався в Identity DB, але запит падав через невдале відправлення вітального листа.

### [ВИКОНАНО 2026-07-03] Оновлення IoT-емулятора та покращення сповіщень у мобільному застосунку

- **Збереження секрету пристрою (ESP32):** Додано використання енергонезалежної пам'яті NVS (`Preferences.h`) в емуляторі IoT. Секретний ключ пристрою зберігається у флеш-пам'яті при першому `claim` і зчитується автоматично при перезапусках, що запобігає виникненню помилок `409 Conflict` на бекенді.
- **Скрипт-симулятор ферми пристроїв:** Створено Node.js скрипт `device_farm_simulator.js` для паралельного моделювання 3-х різних датчиків із власними діапазонами температур та вологості (холодильник, склад, морозилка), що спрощує тестування та демонстрацію.
- **Прискорення фонового моніторингу та запобігання дублюванню (Backend):** 
  - Зменшено інтервал перевірки умов у фоновому сервісі бекенду `StorageConditionMonitoringService` з 60 до 5 секунд для швидшого реагування на відхилення від норм.
  - **Виправлено логічну помилку дублювання інцидентів**: Додано порівняння дати останнього отриманого показання телеметрії (`condition.Timestamp`) з датою створення останнього інциденту (`lastIncident.CreatedAt`). Якщо нові дані не надходять (сенсор вимкнено), сервіс більше не створює нові інциденти-дублікати після того, як менеджер вручну закрив попередній.
- **Бадж інцидентів та системні сповіщення (Android):**
  - Додано динамічне оновлення баджа з кількістю активних інцидентів біля пункту меню «Інциденти» в `MainActivity.kt`.
  - Додано дозвіл `POST_NOTIFICATIONS` та запит прав для Android 13+.
  - Створено `NotificationChannel` високої важливості та інтегровано фоновий цикл перевірки нових сповіщень (кожні 10 секунд). Нові сповіщення тепер успішно виводяться безпосередньо у системну шторку Android зі звуком та вібрацією.
  - **Миттєве авто-опитування (Polling) списків**: Додано фонове опитування сервера кожні 5 секунд у `StorageIncidentsViewModel` та оновлено таймер `NotificationsViewModel` до 5 секунд. Тепер сторінки сповіщень та інцидентів оновлюються автоматично без необхідності перезаходу.
  - **Спрощення вирішення інцидентів (без модальних вікон)**: Повністю прибрано діалогове вікно підтвердження. Тепер інцидент вирішується миттєво по кліку на кнопку «Вирішити» (або «Закрити») прямо в картці інциденту на екрані `IncidentsFragment.kt`, супроводжуючись коротким повідомленням Toast.
  - **Виправлено збій активації/деактивації пристроїв**: Виправлено помилку `JsonReader.setLenient(true) malformed JSON` при зміні статусу пристрою. Сервер на запит `/api/iotdevice/setstatus/{id}` повертає звичайний текст (plain text) замість JSON. Тип повернення методу `setDeviceStatus` у `ApiService.kt` змінено на `Response<okhttp3.ResponseBody>`, що усунуло помилку десеріалізації.
...
- **Оновлення аудиту мобільного клієнта:** Проведено повторний порівняльний аудит Android-застосунку з бекендом та фронтендом. Усунуто більшість критичних прогалин (реалізовано форму відновлення/скидання пароля, сторінку профілю `/me`, фільтрацію за типом у списку препаратів та інцидентів, видалення інцидентів по довгому кліку). За запитом користувача відновлено бічне меню навігації (`DrawerLayout`) та покращено адаптивність розмітки при зміні орієнтації екрана. Результати зафіксовані у `MOBILE_AUDIT_REPORT.md`.
...
- **Основні зміни:**
  ...
  - **Уточнення обмежень зберігання:**
    ...
    - Оновлено підказки in TextInputLayout відповідних макетів для забезпечення чіткого розмежування показників температури та вологості.
- **Статус тестування та збірки:**
  - Збірка проєкту: ✅ Успішно скомпільовано.
  - Тести: ✅ Усі 11 unit-тестів проходять успішно (`.\gradlew testDebugUnitTest`).
- **Статус:** ✅ UX покращення для створення/редагування препаратів успішно інтегровано.

### Запис 27 — Уніфікація навігації: додавання кнопки "Назад" на всіх окремих екранах (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  1. Додати навігаційну стрілочку "Назад" (Navigation Icon) до Toolbar на всіх окремих сторінках додатку, які відкриваються як окремі Activity і не мають бічного меню навігації (створення, редагування, детальний перегляд).
- **Переглянуті файли / модулі:**
  - `Mobile/MedicationManagement` (`res/layout/activity_medicine_details.xml`, `MedicineDetailsActivity.kt`, `DeviceDetailsActivity.kt`, `AddMedicineActivity.kt`, `EditMedicineActivity.kt`, `res/values/strings.xml`, `res/values-uk/strings.xml`).
- **Основні зміни:**
  - **Сторінка деталей препарату (MedicineDetailsActivity):**
    - Макет `activity_medicine_details.xml` перероблено: додано `CoordinatorLayout`, `AppBarLayout` та `MaterialToolbar` зі значенням `@string/medicine_details_title`. Весь контент загорнуто у `NestedScrollView` з підтримкою темної теми (`?attr/colorSurface`).
    - В `MedicineDetailsActivity.kt` ініціалізовано Toolbar як SupportActionBar, увімкнено стрілочку назад `setDisplayHomeAsUpEnabled(true)` та додано обробник `setNavigationOnClickListener { finish() }`.
  - **Інші сторінки:**
    - В `DeviceDetailsActivity.kt`, `AddMedicineActivity.kt` та `EditMedicineActivity.kt` ініціалізовано Toolbar, додано стрілочку назад та обробник завершення Activity (`finish()`).
    - Додано нові локалізовані ресурси (`medicine_details_title`) для заголовку деталей препарату в англійську та українську локалізації.
- **Статус тестування та збірки:**
  - Збірка проєкту: ✅ Успішно скомпільовано (`.\gradlew.bat compileDebugKotlin`).
- **Статус:** ✅ Кнопку "Назад" інтегровано на всі окремі екрани застосунку.

### Запис 28 — Локалізація ролей, типів локацій та загальних повідомлень про помилки (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  1. Локалізувати відображення типів локацій в картках локацій зберігання.
  2. Локалізувати відображення ролей користувачів (Адміністратор, Менеджер, Користувач) у профілі та списку користувачів.
  3. Видалити захардкоджені українські рядки дат придатності в `MedicineAdapter.kt`.
  4. Забезпечити повну симетричність ресурсних файлів `strings.xml` (EN та UK) та локалізувати повідомлення про помилки у ViewModels.
- **Переглянуті файли / модулі:**
  - `Mobile/MedicationManagement` (`res/values/strings.xml`, `res/values-uk/strings.xml`, `StorageLocationAdapter.kt`, `UserAdapter.kt`, `MedicineAdapter.kt`, `ProfileFragment.kt`, `UsersViewModel.kt`, `StorageLocationsViewModel.kt`, `StorageIncidentsViewModel.kt`, `SensorsViewModel.kt`, `NotificationsViewModel.kt`, `MedicinesViewModel.kt`).
- **Основні зміни:**
  - **Локалізація типів локацій:**
    - В `StorageLocationAdapter.kt` тип локації тепер динамічно конвертується у локалізований рядок відповідно до значень (`refrigerator`, `shelf`, `warehouse`, `cabinet`, `other`).
  - **Локалізація ролей:**
    - Додано рядкові ресурси для ролей (`role_administrator`, `role_manager`, `role_user`) в обидві локалізації.
    - В `ProfileFragment.kt` та `UserAdapter.kt` змінено пряме виведення ролей з API на локалізовані відповідники.
  - **Локалізація термінів придатності ліків:**
    - Додано ресурси `medicine_valid_until` та `medicine_expired_uppercase`.
    - Замінено захардкоджені українські рядки "Дійсний до" та "ПРОТЕРМІНОВАНО" в `MedicineAdapter.kt` на використання ресурсів.
  - **Виправлення та аудит ресурсів:**
    - Додано переклад для `no_audit_logs` та перекладено `medicine_invalid_id` в українській локалі. За допомогою скрипта перевірено симетричність ключів.
    - Виправлено захардкоджені українські рядки повідомлень помилок у всіх класах ViewModels (`UsersViewModel`, `StorageLocationsViewModel`, `StorageIncidentsViewModel`, `SensorsViewModel`, `NotificationsViewModel`, `MedicinesViewModel`) на виклики `context.getString(R.string.error_...)`.
- **Статус тестування та збірки:**
  - Збірка проєкту: ✅ Успішно скомпільовано (`.\gradlew.bat compileDebugKotlin` закінчився з кодом 0).
- **Статус:** ✅ Локалізацію типів локацій, ролей та повідомлень про помилки успішно виконано.

### Запис 29 — Оптимізація та локалізація інтерфейсу журналу логів аудиту (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  1. Замінити текстове поле фільтрації за дією ("Action") у веб-інтерфейсі та мобільному застосунку на структурований випадаючий список.
  2. Забезпечити підтримку локалізації для списку вибору дій як на фронтенді, так і в мобільному застосунку.
  3. Локалізувати повідомлення про помилки завантаження у `AuditLogViewModel.kt` мобільного застосунку.
- **Переглянуті файли / модулі:**
  - Web Frontend: `Frontend/src/pages/AuditLogPage.tsx`
  - Mobile Application: `Mobile/MedicationManagement` (`fragment_audit_log.xml`, `AuditLogFragment.kt`, `AuditLogAdapter.kt`, `AuditLogViewModel.kt`)
- **Основні зміни:**
  - **Веб-фронтенд (React):**
    - Створено константний відсортований масив `AUDIT_ACTIONS`, який містить усі системні дії аудиту.
    - Текстове поле введення дії замінено на елемент `<select>` із відображенням локалізованих назв дій через функцію `translateAuditAction(act, t)`.
  - **Мобільний застосунок (Android / Kotlin):**
    - Змінено розмітку фільтру в `fragment_audit_log.xml` — замість `TextInputLayout` з `TextInputEditText` встановлено exposed dropdown меню `AutoCompleteTextView` зі стилем `OutlinedBox.Dense.ExposedDropdownMenu`.
    - В `AuditLogFragment.kt` налаштовано `ArrayAdapter` для заповнення списку дій та локалізованого вибору. Реалізовано збереження англомовного значення для запитів до API та відображення перекладеного значення користувачеві.
    - Оновлено `AuditLogAdapter.kt` для динамічного відображення локалізованих назв дій у списку логів.
    - Виправлено захардкоджені повідомлення про помилки у `AuditLogViewModel.kt` з використанням загальних ресурсів `error_loading` та `error_network`.
- **Статус тестування та збірки:**
  - Фронтенд: ✅ Успішна перевірка типів TypeScript (`npx tsc --noEmit`).
  - Мобільний додаток: ✅ Успішна компіляція Kotlin/Java (`./gradlew compileDebugSources`).
- **Статус:** ✅ Завдання виконано в повному обсязі, інтерфейс фільтрації логів аудиту оптимізовано та локалізовано.

### Запис 30 — Генерація повного набору тестових даних (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  1. Наповнити базу даних MedicationManagement тестовими даними для перевірки фільтрації у логах аудиту та відображення списків.
  2. Додати щонайменше 20 препаратів з різними категоріями та статусами.
  3. Додати 5 місць зберігання (StorageLocations) та 5 IoT-пристроїв (IoTDevices) зі зв'язками між ними.
  4. Додати 20 записів журналів аудиту (AuditLogs) з реальними системними діями для тестування Exposed Dropdown Menu.
- **Основні зміни:**
  - Створено SQL-скрипт `seed_data.sql` у папці scratch, який динамічно отримує `OrganizationId` адміністратора `admin@medstorage.com` та пов'язує з ним усі сутності.
  - Написано та успішно виконано PowerShell скрипт `run_seed.ps1` для виконання SQL-запитів сідування безпосередньо через ADO.NET до локальної бази даних MS SQL.
  - Успішно вставлено:
    - 5 IoT-пристроїв (`DEV-01` - `DEV-05`)
    - 5 локацій зберігання (`Refrigerator A/B`, `Main Warehouse`, `Narcotic Vault`, `Dispensing Shelf`)
    - 20 препаратів (14 Active, 3 Expired, 2 Disposed, 1 Recalled)
    - 20 записів аудиту з діями: `Login`, `Register`, `Create Storage Location`, `Device Claim`, `Create Medicine`, `Receive Medicine`, `Move Medicine`, `Incident Created`, `Auto Expired` тощо.
- **Статус:** ✅ Базу даних успішно наповнено тестовими даними, система готова до демонстрації та тестування.

### Запис 31 — Виправлення помилки 400 Bad Request при створенні локації (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  - Виправити помилку `400 Bad Request` при відправці POST-запиту на створення нової локації зберігання (`/api/storagelocation`).
- **Переглянуті файли / модулі:**
  - Web Frontend: `Frontend/src/pages/StorageLocationsPage.tsx`
  - Web Backend: `Controllers/StorageLocationController.cs`, `Models/DTOs/ResponseDTOs.cs`
- **Основні зміни:**
  - **Аналіз помилки:** Виявлено, що на бекенді DTO-запис `StorageLocationDto` очікує обов'язкове (non-nullable) поле `LocationType`. Проте на фронтенді стан форми `form` ініціалізувався порожнім об'єктом `{}`. Якщо користувач не змінював дефолтне значення селекту, поле `locationType` не записувалося в стан і не передавалося в JSON-запиті. Це викликало помилку валідації моделі на стороні сервера.
  - **Виправлення:** У формі створення локації [StorageLocationsPage.tsx](file:///d:/Learning/Diploma/Frontend/src/pages/StorageLocationsPage.tsx) стан `form` тепер ініціалізується початковим значенням `{ locationType: 'Refrigerator', ...initial }`. Це гарантує присутність обов'язкового поля в JSON при будь-яких діях користувача.
- **Статус:** ✅ Помилку `400 Bad Request` успішно усунено, створення локацій працює коректно.

### Запис 32 — Вирішення проблеми з авторизацією ферми IoT-емуляторів (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  - Усунути помилку авторизації `HTTP 401: Invalid device credentials` при запуску симулятора датчиків `device_farm_simulator.js`.
- **Переглянуті файли / модулі:**
  - База даних: `seed_data.sql` (папка scratch)
  - IoT Emulator: `d:\Learning\Diploma\IoTEmulate\device_farm_simulator.js`, `d:\Learning\Diploma\IoTEmulate\farm_secrets.json`
- **Основні зміни:**
  - **Аналіз проблеми:** Виявлено, що симулятор `device_farm_simulator.js` жорстко використовує ідентифікатори датчиків `WOKWI-SENSOR-A1`, `WOKWI-SENSOR-B2` та `WOKWI-SENSOR-C3`. Проте при первинному сідуванні бази даних створювалися пристрої з ID `DEV-01` .. `DEV-05`. Оскільки пристроїв симулятора не було в БД, запит авторизації застарілих секретів з файлу `farm_secrets.json` повертав 401. Також спроба Claim для неіснуючих ID пристроїв повертала помилку "not found".
  - **Виправлення:**
    1. Оновлено SQL-скрипт `seed_data.sql` — додано створення пристроїв `WOKWI-SENSOR-A1`, `WOKWI-SENSOR-B2` та `WOKWI-SENSOR-C3` з пустими секретами (`DeviceSecretHash = ''`) та зв'язано їх із відповідними локаціями зберігання (`Refrigerator A/B`, `Main Warehouse`).
    2. Повторно виконано сідування бази даних.
    3. Очищено застарілі секрети у файлі `farm_secrets.json` (записано порожній об'єкт `{}`).
  - **Результат:** При наступному запуску симулятор успішно заклаймив пристрої, автоматично отримав нові секрети з бекенду, зберіг їх та почав успішно відправляти телеметрію температури й вологості.
- **Статус:** ✅ Ферма IoT-емуляторів працює стабільно і коректно передає дані.

### Запис 33 — Реструктуризація фільтрації логів аудиту за категоріями (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  - Зменшити і згрупувати велику кількість фільтрацій дій в аудиті логів до 5 високорівневих бізнес-категорій:
    1. Дії над препаратами (будь-які).
    2. Дії над локаціями (будь-які).
    3. Створення та закриття інцидентів.
    4. Дії над девайсами (будь-які).
    5. Дії над користувачами (вхід, створення/видалення менеджерів тощо).
- **Переглянуті файли / модулі:**
  - Web Backend: `Services/ServiceAuditLog.cs`
  - Web Frontend: `Frontend/src/pages/AuditLogPage.tsx`, `Frontend/src/contexts/LocaleContext.tsx`
  - Mobile App: `Mobile/MedicationManagement/.../ui/AuditLogFragment.kt`, `.../res/values-uk/strings.xml`, `.../res/values/strings.xml`
- **Основні зміни:**
  - **Бекенд:** У `ServiceAuditLog.cs` метод `GetLogs` тепер підтримує спеціальні категоріальні значення параметра `action`:
    - `medicine_actions` — фільтрація за ключовими словами дій (`Medicine`, `Expired`, `Lifecycle`, `Receive`, `Issue`, `Move`, `Dispose`) або типом сутності `Medicine`/`MedicineLifecycleEvent`.
    - `location_actions` — фільтрація за діями над локаціями (`Location`) або типом сутності `StorageLocation`.
    - `incident_actions` — фільтрація за діями над інцидентами (`Incident`) або типом сутності `StorageIncident`.
    - `device_actions` — фільтрація за пристроями (`Device`, `Sensor`) або типом сутності `IoTDevice`.
    - `user_actions` — фільтрація за діями користувачів, автентифікації та ролей (`User`, `Manager`, `Login`, `Register`, `Email`, `Password`, `Role`) або типом сутності `ApplicationUser`.
  - **Фронтенд:**
    - Оновлено список `AUDIT_ACTIONS` в `AuditLogPage.tsx` на ці 5 категорій.
    - Оновлено перекладач `translateAuditAction` та додано локалізаційні рядки для обох мов (UK, EN) в `LocaleContext.tsx`.
  - **Мобільний додаток:**
    - Оновлено список фільтрів у випадаючому списку `AuditLogFragment.kt`.
    - Додано ресурси локалізації для нових категорій до файлів `strings.xml` (для `values-uk` та `values`).
    - Метод `getActionDisplayName` переписано для зчитування рядків безпосередньо з системних ресурсів Android через `getString(R.string...)`.
- **Статус:** ✅ Нова категоріальна фільтрація логів аудиту успішно реалізована і перевірена. Проекти бекенду, фронтенду та Android клієнта успішно збираються і компілюються.

### Запис 34 — Валідація відправки пошти через SendGrid SMTP (виконано 2026-07-04)

- **Дата:** 2026-07-04
- **Завдання:**
  - Надіслати тестовий лист із кодом автентифікації на адресу `sendgridtesting@gmail.com` через налаштовану в системі службу SendGrid SMTP.
- **Основні зміни:**
  - **Аналіз:** Утиліта `SmtpTester` була використана для перевірки з'єднання з SendGrid SMTP.
  - **Результат:** Здійснено два тестових запуски з портами `2525` та `587`. Листи з кодом `hybp6W5djaCKuKDe` та темою `hybp6W5djaCKuKDe` були успішно відправлені від імені `noreply@em8784.medstorage.me` на адресу `sendgridtesting@gmail.com`. Обидва запити виконано успішно, сервіс SendGrid підтвердив доставку листа без жодних помилок з'єднання чи авторизації.
- **Статус:** ✅ Надсилання пошти через SendGrid SMTP працює коректно і стабільно.

### Запис 35 — Впровадження локального адміністрування організацій (OrganizationAdmin) та ізоляції даних (виконано 2026-07-05)

- **Дата:** 2026-07-05
- **Завдання:**
  1. Реалізувати повноцінне локальне адміністрування в межах організацій за допомогою нової ролі `OrganizationAdmin`.
  2. Додати роль `OrganizationAdmin` до переліку дозволених у політиках авторизації (`[Authorize(Roles = ...)]`) для всіх основних контролерів і сервісів.
  3. Надати доступ `OrganizationAdmin` до журналу логів аудиту (`AuditLogController`), але обмежити відображення виключно його організацією.
  4. Забезпечити підтримку поля `OrganizationId` у DTO та логіці мапінгу для адміністративного призначення ресурсів.
  5. Оновити веб-інтерфейс (SPA) та мобільний застосунок для підтримки нової ролі `OrganizationAdmin`.
- **Переглянуті файли / модулі:**
  - Backend: `StorageLocationController.cs`, `StorageIncidentController.cs`, `StorageConditionController.cs`, `NotificationController.cs`, `MedicineLifecycleController.cs`, `MedicineController.cs`, `IoTDeviceController.cs`, `AuditLogController.cs`, `AuthController.cs`, `ResponseDTOs.cs`, `MappingExtensions.cs`.
  - Frontend: `Frontend/src/pages/UsersPage.tsx`, `Frontend/src/contexts/AuthContext.tsx`, `Frontend/src/contexts/LocaleContext.tsx`.
  - Mobile: `RoleHelper.kt`, `ProfileFragment.kt`, `ApiService.kt`, `UserAdapter.kt`, `UsersFragment.kt`, `res/values/strings.xml`, `res/values-uk/strings.xml`.
- **Основні зміни:**
  - **Backend:**
    - Додано роль `OrganizationAdmin` до атрибутів `[Authorize(Roles = ...)]` у всіх контролерах бізнес-логіки.
    - Виправлено помилку збірки в `AuthController.cs` шляхом підключення правильного простору імен `MedicationManagement.DBContext`.
    - Оновлено мапінг та структуру DTO (додано опціональне поле `OrganizationId` для можливості призначення ресурсів організаціям).
  - **Frontend:**
    - Оновлено `AuthContext.tsx`: додано нову роль до визначень, а `isAdmin` тепер є істинним як для глобального `Administrator`, так і для локального `OrganizationAdmin`.
    - Оновлено сторінку `UsersPage.tsx`:
      - Додано переклад та відображення бейджа `OrganizationAdmin` (як українською, так і англійською).
      - Налаштовано форму створення менеджера: для глобального адміна тепер відображається вибір ролі (`Manager` або `OrganizationAdmin`) та випадаючий список існуючих організацій із можливістю введення назви нової організації.
      - Відображається реальна назва організації `u.organizationName` у таблиці користувачів.
  - **Mobile Application:**
    - Оновлено `RoleHelper.kt`: перевірка `isAdmin(role)` тепер повертає `true` і для `OrganizationAdmin`. Також оновлено інші перевірки прав доступу (`isManager`, `isUser`, `hasFullAccess`).
    - Оновлено `UserDto` в `ApiService.kt` (додано поле `organizationName`), завдяки чому в `UserAdapter.kt` тепер виводиться зрозуміла назва організації замість її UUID.
    - Оновлено форму створення користувача в `UsersFragment.kt`: якщо поточний користувач має роль `OrganizationAdmin`, поле вводу ID організації блокується, гарантуючи, що менеджер може бути створений лише у межах його організації.
