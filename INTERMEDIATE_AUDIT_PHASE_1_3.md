...
**Тимчасовий захист:** `AddNewtonsoftJson()` у `Program.cs` без налаштування `ReferenceLoopHandling.Ignore` — існує ризик 500 помилок при циклічних навігаційних властивостях (наприклад, `StorageLocation.Medicines[].StorageLocation`).

**Рішення для Фази 4:** Ввести Response DTO для кожного ресурсу, де сенс. Мінімальний приклад:
```csharp
...
|---|---|---|
| CORS | ❌ Не налаштовано | Frontend-SPA не зможе робити запити без CORS-заголовків |
...
| Response DTO | ❌ Відсутній | Сирі entity — ризик циклічної серіалізації |
...
|---|---|---|
| Технічний борг | 15 | 2 |
| Відсутні API-фіче (для Фази 4) | 3 (CORS, DTO, Pagination) | 1 (CORS) |
| Готових ендпоінтів | 34 | — |
| Покрито тестами | 0 | — |

**Найважливіші дії перед Фазою 4:**
1. 🔴 Додати CORS policy (без цього SPA буде повністю заблокований)
2. 🟡 Виправити `GetExpiringMedicines` (не включати вже протерміновані)
3. 🟡 Додати `AsNoTracking()` до read-запитів
4. 🟡 Прибрати Cookie-автентифікацію або задокументувати намір
5. 🟢 Видалити або перенести константи з `IoTEmulate/main.cpp`
